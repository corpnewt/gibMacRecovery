...
# noinspection PyInconsistentReturns,SqlNoDataSourceInspection
class GibMacRecovery:
    def __init__(self):
        ...
        self.boards_path = os.path.join(
            os.path.dirname(
                os.path.realpath(__file__)
            ),
            "Scripts",
            os.path.basename(self.boards_url)
        )
        self.macrecovery_path = os.path.join(
            os.path.dirname(
                os.path.realpath(__file__)
            ),
            "Scripts",
            os.path.basename(self.macrecovery_url)
        )
        ...
        if os.name == "nt":
            ...
            self.min_h = 30
        if (
                not os.path.exists(self.boards_path) or
                not os.path.exists(self.macrecovery_path) or
                not os.path.exists(self.recovery_path)
    ...
    def update_macrecovery(self):
        ...
        try:
            for path, url in (
                (self.boards_path, self.boards_url),
                (self.macrecovery_path, self.macrecovery_url),
                (self.recovery_path, self.recovery_url)
            ):
            ...
            for mac in self.boards:
                if self.boards[mac].lower() == os_list[menu]:
                    ...
                    return (
                        ...
                        mac,
                        "00000000000000000",
                        "latest" if "latest" in os_list[menu].lower() else "default"
                    )

    def details(self):
        return "\n".join(
            [
                f"     Target macOS: {self.target_macos}",
                f"  Target Board ID: {self.target_mac}",
                f"       Target MLB: {self.target_mlb}",
                f"          OS Type: {self.latest_default}"
            ]
        )
        ...
        while True:
            ...
            if len(self.recovery[macos]) > 1:
                while True:
                    lines = [
                        "",
                        "{} has {:,} urls:".format(macos, len(self.recovery[macos])),
                        ""
                    ]
    ...
    def main(self):
        ...
        print("")
        print(
            "1. {} macrecovery.py, boards.json, and recovery_urls.txt".format(
                "Update" if all(
                    (
